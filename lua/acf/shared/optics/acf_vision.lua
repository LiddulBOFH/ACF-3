...
if CLIENT then
    ...
    function ACF_Vision.nightvision_start()
        ...
        hook.Add("RenderScreenspaceEffects", "ACF_Vision", function()
            ...
            --DrawSharpen(1,0.5)
            DrawTexturize(0,Material("postprocessing/acf_nightvision2.png"))
        end)
    ...
    function ACF_Vision.enable(mode) -- actually toggles the whole system
        ...
        if mode == 1 then ACF_Vision.nightvision_start()
    ...
    function ACF_Vision.AdjustIntensity()
        ...
        if math.abs(CenterPoint) > 0.05 then
            ACF_Vision.intensitymod = math.Clamp(ACF_Vision.intensitymod + (CenterPoint / 4),-5,5)
        end

        if ACF_Vision.mode == 1 then
            ACF_Vision.nv_colmod["$pp_colour_contrast"] = 4 + (ACF_Vision.intensitymod * 8)
    ...
    function ACF_Vision.enable(player, mode)
        player:SetNWInt("ACF_Vision_Mode",math.Clamp(math.floor(mode),0,4))
    end
end
