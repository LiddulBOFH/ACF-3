...
local Armors   = Classes.ArmorTypes.GetEntries()
...
--[[ All capable of getting the surface area of various parts of a mesh, just unused but the work is done
...
local function TraceThroughObject(trace,Bullet)
	...
	util.TraceLine({start = enter - dir,endpos = enter + dir,
		filter = function(te)
			if te ~= ent then
...
function ACF.Armor.CalculateHealthMod(density) -- TODO: finish
	local HealthMod = 1100
	local scale = (density / 7.84) - 1

	if density < (7.84 * 0.25) then -- poor health for super low density?
		HealthMod = HealthMod * (1 + scale) * 0.75
	elseif density > (7.84 * 1.2) then
		HealthMod = HealthMod * (1 + (scale * 0.5))
	else -- inside the band
		HealthMod = HealthMod * (1 + (scale * 0.5))
	end

	return ((math.Clamp(HealthMod,1,1400) * 3) + 1100) / 4 -- average it out with 3 times of the prop's health mod and 1 time of RHA so everything has a bit of a chance
end
print(ACF.Armor.CalculateHealthMod(9.4))
print(ACF.Armor.CalculateHealthMod(9.5))
...
function ACF.Armor.CalculateHealth(ent,density,volumeOverride)
	...
	if volumeOverride then
		return math.max(math.Round(volumeOverride * ACF.Armor.CalculateHealthMod(density),1),1)
	else
